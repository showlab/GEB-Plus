...
from utils.evaluation_utils import evaluate_on_retrieval
from datasets.retrieval_dataset import RetrievalDataset, RetrievalCorpusDataset
...
def save_checkpoint(model, tokenizer, args, epoch, iteration, num_trial=10):
    ...
    checkpoint_dir = os.path.join(args.output_dir, 'ret_checkpoint-{}-{}-{}-{}'.format(
...
def retrieval_dataloader(args, tokenizer, split, corpus=False):
    if corpus:
        dataset = RetrievalCorpusDataset(args, split)
    else:
        dataset = RetrievalDataset(args, tokenizer, split)
...
def train(args, train_dataloader, val_dataloader, val_corpus_dataloader, model, tokenizer):
    ...
    logger.info("***** Running training for Video Retrieval *****")
    ...
    for epoch in range(int(args.num_train_epochs)):
        for step, (boundary_ids, batch) in enumerate(train_dataloader):
            ...
            loss = norm_softmax_loss(args, outputs)
            loss.backward()
            ...
            if (args.save_steps > 0 and global_step % args.save_steps == 0) or global_step == t_total:
                ...
                if args.evaluate_during_training:
                    ...
                    evaluate_file = evaluate(args, val_dataloader, val_corpus_dataloader, model, checkpoint_dir)
                    with open(evaluate_file, 'r') as f:
                        res = json.load(f)
                    best_score = max(best_score, res['mAP'])
                    res['epoch'] = epoch
                    res['global_step'] = step
                    res['best_mAP'] = best_score
                    eval_log.append(res)
...
def evaluate(args, test_dataloader, test_corpus_dataloader, model, output_dir):
    ...
    with torch.no_grad():
        ...
        for step, (boundary_ids, batch) in tqdm(enumerate(test_dataloader)):
            ...
            cap_embed_list.append(cap_embedding)
            for b_id in boundary_ids:
                query_ids.append(b_id)

        all_query = torch.cat(cap_embed_list, dim=0)

        for step, (boundary_ids, batch) in tqdm(enumerate(test_corpus_dataloader)):
            batch = tuple(t.to(args.device) for t in batch)
            ctx_inputs = {
            ...
            time_meter += time.time() - tic
            ctx_embed_list.append(ctx_embedding)
            for b_id in boundary_ids:
                ctx_ids.append(b_id)

        all_ctx = torch.cat(ctx_embed_list, dim=0)
        sims = sim_matrix(all_query, all_ctx).cpu().numpy()

        rank, metric = evaluate_on_retrieval(sims, query_ids, ctx_ids, outfile=[predict_file, evaluate_file])
...
def main():
    ...
    parser.add_argument('--gpu_ids', type=str, default='5 6 7')
    parser.add_argument("--per_gpu_train_batch_size", default=108, type=int,
                        help="Batch size per GPU/CPU for training.")
    parser.add_argument("--per_gpu_eval_batch_size", default=108, type=int,
    ...
    logger = setup_logger("retrieval", output_dir)
    ...
    if args.do_train:
        train_dataloader = retrieval_dataloader(args, tokenizer, split='train')
        val_dataloader = retrieval_dataloader(args, tokenizer, split='val')
        val_corpus_dataloader = retrieval_dataloader(args, tokenizer, split='val', corpus=True)
        last_checkpoint = train(args, train_dataloader, val_dataloader, val_corpus_dataloader, model, tokenizer)

        # test the last checkpoint after training
        if args.do_test:
            logger.info("Evaluate for Retrieval after Training")
            test_dataloader = retrieval_dataloader(args, tokenizer, split='test')
            test_corpus_dataloader = retrieval_dataloader(args, tokenizer, split='test', corpus=True)
            evaluate(args, test_dataloader, test_corpus_dataloader, model, last_checkpoint)

    # inference and evaluation
    elif args.do_test or args.do_eval:
        logger.info("Evaluate for Retrieval")
        test_dataloader = retrieval_dataloader(args, tokenizer, split='test')
        test_corpus_dataloader = retrieval_dataloader(args, tokenizer, split='test', corpus=True)
        ...
        if not args.do_eval:
            ...
            evaluate(args, test_dataloader, test_corpus_dataloader, model, checkpoint)
